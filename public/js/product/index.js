(function() {
  ...
  function buttonBasket() {
    ...
    /*$(".button-basket-outside  .cartReplacerTotalSum").text(
      String($.Cart.computeTotalCost()).replace(
        /(\d(?=\d{4,})|\d\d)(?=(\d\d\d)+([^\d]|$))/g,
        "$1 "
      )
    );

    $(".container-with-total-cost  .cartReplacerTotalSum").text(
      String($.Cart.computeTotalCost()).replace(
        /(\d(?=\d{4,})|\d\d)(?=(\d\d\d)+([^\d]|$))/g,
        "$1 "
      )
    ...
    /*if (window.matchMedia("(min-width: 992px)").matches) {
      document.querySelector(
        ".button-basket-outside .cartReplacerCount"
      ).innerText = $.Cart.getCount() === 0 ? "Нет" : $.Cart.getCount();
      document.querySelector(
        ".button-basket-outside .counter-value .counter-postfix"
      ).innerHTML = wordDecline($.Cart.getCount(), [
        "товар",
        "товара",
        "товаров"
      ]);
...
const postifxRepleaceElement = document.querySelector(
  ".button-basket-outside .counter-value .counter-postfix"
);

const handleMutations = throttle(mutation => {
  sumReplaceElements.forEach(el => handleSumReplacer(el));
  /*postifxRepleaceElement.innerHTML = wordDecline($.Cart.getCount(), [
    "товар",
    "товара",
    "товаров"
  ]);*/
...
function scrollSliderPictures(event, actionType = "plus") {
  ...
  const container = document.querySelector(
    ".product-slider-nav-images-container"
  );
  ...
  const minTranslateY =
    container.children.length * parseFloat(buttonHeight) +
    (container.children.length - 1) * parseFloat(buttonGap) -
    parseFloat(
      getComputedStyle(
        container.closest(".product-slider-nav-images")
      ).getPropertyValue("max-height")
    );

  if (valueTranslateY <= -minTranslateY) cloneNodes();
  else if (valueTranslateY >= 0) valueTranslateY = -minTranslateY;

  container.style.transform = `translateY(${valueTranslateY}px)`;
  container.setAttribute("data-translate", valueTranslateY);

  function cloneNodes() {
    const copyNodes = document
      .querySelector(".product-slider-nav-images-container")
      .cloneNode(true);
    [...copyNodes.children].forEach(el =>
      document
        .querySelector(".product-slider-nav-images-container")
        .appendChild(el)
    );
...
if (window.matchMedia("(max-width: 991px)").matches)
  [
    ...document.querySelectorAll(".details-block > input[type='radio']")
